...
std::vector<Entity::Track> Controller::YaController::fetch_tracks_batches(const std::string& token, const std::vector<std::string>& track_names)
{
    std::vector<std::string> bytes;

    //:)
    this->_client->getTracksByStream(token, const_cast<std::vector<std::string>&>(track_names), bytes);
...
int Controller::YaController::getTracksNames(const std::string& tkn, std::vector<std::string>& vec){
    int code = this->_client->getList(tkn, vec);
    return code;
}
//TODO
